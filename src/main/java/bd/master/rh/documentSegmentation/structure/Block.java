...
public class Block implements Comparable{
	...
	public Block(Page page, SortedSet<Block> currentBlock) {
		...
		fillFragmentsIfNull();
		float xTopLeftCorner =  subBlocks.stream().map(b -> b.BoundingBox.getxTopLeftCorner()).min(Float::compareTo).get();
		float xBottomRightCorner= subBlocks.stream().map(b -> b.BoundingBox.getxBottomRightCorner()).max(Float::compareTo).get();
		float yTopLeftCorner = subBlocks.stream().map(b -> b.BoundingBox.getyTopLeftCorner()).min(Float::compareTo).get();
		float yBottomRightCorner= subBlocks.stream().map(b -> b.BoundingBox.getyBottomRightCorner()).max(Float::compareTo).get();
	...
	public Block(Page page, ArrayList<TextBlock> arrayList) {
		...
		float xTopLeftCorner =  fragments.stream().map(f -> f.getPosition().getBoxCoord().getxTopLeftCorner()).min(Float::compareTo).get();
		float xBottomRightCorner= fragments.stream().map(f -> f.getPosition().getBoxCoord().getxBottomRightCorner()).max(Float::compareTo).get();
		float yTopLeftCorner = fragments.stream().map(f -> f.getPosition().getBoxCoord().getyTopLeftCorner()).min(Float::compareTo).get();
		float yBottomRightCorner= fragments.stream().map(f -> f.getPosition().getBoxCoord().getyBottomRightCorner()).max(Float::compareTo).get();
		this.setBoundingBox(new  Box(xTopLeftCorner, xBottomRightCorner, yTopLeftCorner, yBottomRightCorner));
	}
