...
public abstract class FragmentMerger {
	...
	public FragmentMerger(Page page, Iterable<TextBlock> fragments, Comparator<Cluster> clusterComparator) {
		...
		for (TextBlock fragment : fragments) {
			...
			if (c.maxHeight > maxFragmentHeight) {
				maxFragmentHeight = c.maxHeight;
			}
			clusters.add(c);
		}
	...
	private Block getMergeResult() {
		...
		for (Cluster c : clusters) {
			...
			Boolean addingResult = collections.add(entry);
			if( !addingResult) {
				System.out.println("ALERT: entry "+ entry + " belongs already to collections");
			}
